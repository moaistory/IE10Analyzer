...
namespace IE10Analyzer.EDBParser
{
    public class Table
    {
        ...
        public Dictionary<int, Item> getRecord(int recordOffset, int recordSize, int jumpSize){
	        ...
	        foreach(Column column in columnList){
		        ...
		        if(id>=0 && id < 0x80 && id<=lastFixedItemID){//fixed items 파싱
			        ...
			        if(itemID == id){
				        if(firstTaggedDataOffset == 0){ //First
				        ...
				        if(firstTaggedDataOffset == taggedDataItemPosition){ //Last	
        ...
        public List<Dictionary<int, Item>> recoverRecord(){
        ...
        public void addRecord(Dictionary<int, EDBParser.Item> record)
        {
